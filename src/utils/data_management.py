...
from mlproj_manager.problems.supervised_learning.tiny_imagenet.tiny_imagenet_dataloader import TinyImageNetDataSet
...
def get_tiny_imagenet_data(data_path: str, classes: list = None, train: bool = True):
    """
    Loads the Tiny ImageNet data set
    :param data_path: path to where the data is stored in memory
    :param classes: list of classes to load
    :param train: (bool) indicates whether to load the train (True) or the test (False) data
    :return: data set, data loader
    """
    tiny_imagenet_data = TinyImageNetDataSet(root_dir=data_path,
                                             train=train,
                                             classes=classes,
                                             image_normalization="max",
                                             label_preprocessing="one-hot",
                                             use_torch=True)
    set_tiny_imagenet_shape_and_normalization_transforms(tiny_imagenet_data)
    return tiny_imagenet_data

def set_tiny_imagenet_data_augmentation_transforms(tiny_imagenet_data: TinyImageNetDataSet):
    """
    Set the transforms for tiny image net to be:
        reshape -> normalization by mean and std -> random flip -> random crop -> random rotate by 15 degrees
    """
    mean = (0.48043, 0.44820, 0.39756)
    std = (0.27644, 0.26889, 0.28167)
    transformations = [
        ToTensor(swap_color_axis=True),                             # reshape to (C x H x W)
        Normalize(mean=mean, std=std),                              # for each channel, center by mean and divide by std
        RandomHorizontalFlip(p=0.5),                                # random horizontal flip
        RandomCrop(size=64, padding=4, padding_mode="reflect"),     # random crop
        RandomRotator(degrees=(0, 15))                              # random rotation by 15 degrees
    ]
    tiny_imagenet_data.set_transformation(transforms.Compose(transformations))

def set_tiny_imagenet_shape_and_normalization_transforms(tiny_imagenet_data: TinyImageNetDataSet):
    """
    Set the transforms for tiny image net to be:
        reshape -> normalization by mean and std
    """
    mean = (0.48043, 0.44820, 0.39756)
    std = (0.27644, 0.26889, 0.28167)
    transformations = [
...
def get_data_loader(dataset, minibatch_size: int, num_workers: int = 12, shuffle=True):
    """
    Returns a data loader for the given data set
    :param dataset:torchvision Dataset
    :param minibatch_size: size of the mini-batches
    :param num_workers: number of workers for the dataloader
    :param shuffle: whether to shuffle the data
    :return: dataloader
    """
    return DataLoader(dataset, batch_size=minibatch_size, shuffle=shuffle, num_workers=num_workers)
