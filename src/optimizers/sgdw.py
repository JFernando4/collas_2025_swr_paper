"""
This file contains the implementation of the SGDW optimizer. It modifies the SGD implementation from torch version 2.1.1.
The multi-tensor update is not implemented, so it raises an error when trying to use it.
"""
import torch
...
SGDW.__doc__ = r"""Implements stochastic gradient descent (optionally with momentum).
...
def _single_tensor_sgdw(params: List[Tensor],
                        d_p_list: List[Tensor],
                        momentum_buffer_list: List[Optional[Tensor]],
                        *,
                        weight_decay: float,
...
def _multi_tensor_sgdw(params: List[Tensor],
                       grads: List[Tensor],
                       momentum_buffer_list: List[Optional[Tensor]],
                       *,
                       weight_decay: float,
    ...
    for ((device_params, device_grads, device_momentum_buffer_list), indices) in grouped_tensors.values():
        ...
        if weight_decay != 0:
            torch._foreach_mul_(device_params, 1.0 - lr * weight_decay)
